...
def main():
    h, followers = knot(), [knot() for i in range(9)]
    with open("input09.txt", 'r') as insts:
        for line in insts:
            ins = line.split()
            for i in range(int(ins[1])):
                h.move(ins[0])
                followers[0].follow(h.coords)
                for i in range(1, 9):
                    followers[i].follow(followers[i-1].coords)
    print(followers[8].been)
    print(len(followers[8].been))
